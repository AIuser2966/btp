"""Run the full study and write tables + charts to ``results/<currency>/``.

    python run_backtest.py                 # USD investor
    python run_backtest.py --currency INR  # Indian investor, rupee SIP
"""
...
from sip.data import load_returns
from sip.engine import contribution_schedule, run_sip
from sip.metrics import summary_table
from sip.strategies import build_strategies

MONEY_COLS = {"Invested", "Final value"}
RATIO_COLS = {"Wealth multiple", "Sharpe", "Sortino", "Calmar", "Windows"}
...
def main():
    ...
    print(f"{args.currency}: SIP {idx[0]} .. {idx[-1]} ({len(idx)} instalments)")
    ...
    tt_table = summary_table(tt_results)
    ...
    # Charts
    plots.wealth_chart(results, out / "wealth.png", args.currency)
    ...
    report = [
        ...
        f"Best static mix on train (max Sharpe): {w_star}\n", fmt(tt_table), "",
        "## Sensitivity / ablation of the Optimized SIP\n", fmt(sens.reset_index(), index=False), "",
        "## Stress periods (cumulative time-weighted return)\n", fmt(crisis), "",
        "## Hindsight reference\n",
        f"Best static mix with perfect hindsight over the full period (max Sharpe): {hindsight}",
    ]
