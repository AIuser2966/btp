# Can Machine Learning Predict Future Returns and Improve the SIP?

This is an extension to the main study. Everything here comes from
`python run_forecast.py` (USD) and `python run_forecast.py --currency INR`. The full
tables are in `results/<currency>/forecast/forecast_results.md`.
...
## 2. How the forecasting works (`sip/forecast.py`)
...
## 7. Using it: `recommend.py`

```bash
python recommend.py --currency INR --amount 10000
python recommend.py --currency INR --amount 10000 --holdings 250000 180000 120000
python recommend.py --currency INR --amount 10000 --model Ridge
```
